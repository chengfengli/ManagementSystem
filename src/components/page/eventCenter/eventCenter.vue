      ...
      <div class="container">
        <el-row>
          <el-col :span="4">
            <ul class="ul_1">
              <li>
                <div style="font-size: 16px;font-weight: bold;text-align: center;">开放事件</div>
                <div
                  style="font-size: 14px;text-align: center;border-bottom: 1px solid #ccc;padding-bottom: 5px;color: #ccc;margin-bottom: 5px;">
                  看一看、评一评、赞一赞
                </div>
                         
                <div style="line-height: 20px;border-bottom: 1px solid #ccc;padding-bottom: 5px;"><span
                  style="color: red;margin-left: 24px;">特此声明：</span>不良事件系统中所有不良事件仅供医院职工内部职工学习交流，改进自身质量的作用，任何人不得以任何形式将医院不良事件泄露给院外人员。</span>
                </div>
              </li>
            </ul>
            <table class="statistic" border="0" cellspacing="0" cellpadding="0">
              <tr>
                <td>总事件：</td>
                <td>{{statistic.ALLLEVEL}}</td>
              </tr>
              <tr>
                <td>l级事件：</td>
                <td>{{statistic.ONELEVEL}}</td>
              </tr>
              <tr>
                <td>ll级事件：</td>
                <td>{{statistic.TWOLEVEL}}</td>
              </tr>
              <tr>
                <td>lll级事件：</td>
                <td>{{statistic.THREELEVEL}}</td>
              </tr>
              <tr>
                <td>lV级事件：</td>
                <td>{{statistic.FOURELEVEL}}</td>
              </tr>
              <tr>
                <td>个人建议：</td>
                <td>{{statistic.PROPOSALS}}</td>
              </tr>
              <tr>
                <td>点赞：</td>
                <td>{{statistic.THUMBS}}</td>
              </tr>
            </table>
          </el-col>
            ...
            <el-form label-width="80px">
              <el-form-item label="事件类型" size="mini">
                <el-select @change="selectChange" clearable v-model="form.TYPEID">
                  <el-option v-for="eventType in eventTypes" :key="eventType.EVENTTYPEID" :label="eventType.TYPENAME"
                             :value="eventType.EVENTTYPEID"></el-option>
                </el-select>
              </el-form-item>
              <el-form-item label="当事科室" size="mini">
                ...
                <el-col class="line" :span="2"><span href="javascript:vodi(0)" @click="currentDept"
                                                     style="cursor: pointer;color: blue;">本科室事件</span></el-col>
              </el-form-item>
              <el-form-item label="事件等级" size="mini">
                <el-checkbox-group v-model="form.LEVEL" @change="checkboxChange">
                  <el-checkbox v-for="item in eventlevels" :label="item.VALUE" :key="item.VLAUE">{{item.DISPLAY}}
                  </el-checkbox>
                </el-checkbox-group>
              </el-form-item>
            </el-form>
            ...
            <table class="data" cellspacing="0">
              <tr v-for="item in dataList" :keys="item.count" @click="details(item.id)">
                ...
                <td><img class="icon" src="../../../../static/img/public/click.png"><span class="count">({{item.CLICKS}})</span>&nbsp;&nbsp;<img
                  class="icon" src="../../../../static/img/public/comment.png"><span
                  class="count">({{item.PROPOSALS}})</span>&nbsp;&nbsp;<img class="icon"
                                                                            src="../../../../static/img/public/thumb.png"><span
